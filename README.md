...
| `v1.86.0` | Three more from the competitor scan. **How everyone has played it** (Standings, `managerCard`, `SCC.managerReport` pure and tested): each manager in the league rated on the parts they control, what they have picked up off waivers and still hold, and what their trades have made, at today's values, ranked within the league. A manager who has done nothing reads blank rather than bottom, since silence is not a failing. FantasyPros charges for the same idea; nothing else prices it from your league's own transactions. **What this week is worth** (`whatIf`, `SCC.standings` `opts.force`): the playoff simulation run twice more with your game already decided, so the card says "beat them and your playoff chance is 61%, lose and it is 34%". Scores stay realistic; only their order is forced. **Live projections** (`SCC.liveProjection`, `projOf`): Sleeper writes a projection before kickoff and never moves it, so by the third quarter it describes a game that is not happening. A player's number is now his points plus half of what is left once his game is on, and his points once it is over, which is the assumption the win probability already made |
| `v1.85.0` | **Since you last looked** (Today, `changedLine`, `SCC.whatChanged` pure and tested): what is different from the last time the app was closed, a starter newly ruled out first, then new lineup changes, hurt starters and wire targets, with a re-import of the week's rankings above them all. The comparison snapshot (`lookSnapshot`, KEY.look) is frozen for the visit so the list holds still, and rewritten as you go for the next one. Nothing on a first visit and nothing in a new week, which is a new slate rather than a change. **The graded record speaks where the call is made**: once a season has five scored close calls, the close-call notes carry "Titan's close calls are 6 of 11 for you this season", and say plainly that it is the record for close calls as a whole rather than for ones resembling this. **Cold start**: `preconnect` to Sleeper's hosts and `fetchpriority` on the two files the first screen needs took a throttled phone from 4.6s to 4.3s. The rest is volume, not waiting, and the note below says what is left |
| `v1.84.0` | Type, corners and the front door. The stylesheet had grown **42 distinct font sizes** and **10 border radii**, so nothing read as deliberate; they are now 8 type tokens (`--t-2xs` to `--t-2xl`, each step a real jump) and 4 radius tokens, applied across 224 and 65 declarations. Two hierarchy slips the mapping exposed were fixed rather than papered over: the week's record had grown to match the week's points, and the tile number needed the 1.35rem step the scale had skipped. **The website now serves Inter too**, from the file Titan already hosts, so a stranger no longer meets the system font on the home page and Inter in the app. **The home page shows the product**: three screenshots of the demo at phone width, generated by `tools/shots.js` so they can be remade rather than hand-kept. Taking them exposed two flaws now fixed: a changed lineup row coloured each cell instead of the row, drawing the slot column as a stray block, and the waiver and streamer screens opened with six to eight lines of instructions, which moved behind a "how this works" line |
| `v1.83.0` | Three gaps found by scanning what other fantasy tools sell. **What a change is worth** (`changeWorth`, `SCC.lineupSwing`, pure and tested): under the changes on a league card, your chance of winning as the lineup stands against the chance with Titan's, the same opponent both times, and the projected points between them. Yahoo charges for this as Assistant GM's headline. A rank says who is better; this says what it costs you. **How he practised** (`SCC.practiceNote`): Sleeper carries practice participation and a description per player and Titan ignored both, so a Questionable tag now reads "Questionable (Hamstring), did not practise", green when he was a full participant and red when he was not, on every status line and in the projections-disagree note. `fetchDetails` and `applyInjuries` carry it. **Streamers look ahead**: each kicker and defense says how the next three weeks grade (`SCC.scheduleStrength` over the schedule and the season's points allowed by position, since betting lines exist only for this week), so a one-week rental reads differently from someone worth holding |
| `v1.82.0` | No more placeholder league icons (the owner asked whether to remove them; none of his twelve leagues has a picture, so every icon was the same Titan logo repeated beside names that were the only thing telling them apart). A league with a picture of its own, a Sleeper avatar or an ESPN team logo, still shows it with the site badge. A league without one now shows the site letter alone, and nothing at all in the left-hand list, where the line underneath already names the site |
| `v1.81.0` | The list of leagues down the left of a wide window learned a second job (the owner: "on the trade analyzer we add that same league filter for fast swapping"). On a screen that stacks every league it still jumps to one. On a screen that shows one at a time it switches to it, which is the dropdown's work in a single click, with the league showing marked and the page keeping its scroll position so a swap reads as a filter rather than a new page. Trade and Standings have it now, Trade marking a league with completed trades this season and saying how many. Byes gets it with an "All leagues" entry at the top, since that screen can show every league at once. The dropdowns stay for phones, where there is no sidebar |
| `v1.80.0` | Every screen measured on a phone, and the four longest cut down. News printed every story ESPN returned, 5,568px in one block: it now leads with the stories that tag your roster, shows ten, and keeps the rest behind a button (7.6 screens to 2.9). Waivers folds each league's claims with the first open, and the trending pickups show eight before asking (6.4 to 4.4). Rosters folds the bench and the reserve, starters staying open (3.9 to 2.8). Import multiple previews fifteen a position rather than the lot. **And the measurement is now a test**: the UI suite walks every screen at 390px and fails when one goes past its budget, naming the tallest block, so the next ten features can't quietly undo this. Loading states are skeleton rows the height of the real ones instead of a line of text, so a screen keeps its shape while it waits |
| `v1.79.0` | One lineup block, and one set of trade pickers (both the owner's calls). **Lineups** drops the separate recommended list: a league card is now a single table, the recommended lineup beside the one you have, since the two were showing the same nine players. A spot that matches takes one line across both columns (41px on a phone); a spot that changes gets the depth, the photo, the kickoff, the projection, the rank note, the game context, the NEW mark and why that player has it. On a phone the two columns stack and are labelled Start and You have, because three columns of names wrap into a wall at 390px. `recRow`, `rowDepth` and `foldCompare` are gone with it. Measured: the block went 1,575px to 871px on the way, mostly by stopping `.lu-cmp td b` turning every bold word into its own line. **Trade** keeps one set of League and Trade partner pickers, the pair right above the rosters they build, with Clear all beside them; the top pair is gone |
| `v1.78.0` | A pass for density and polish, measured on a 390px phone against the live demo. Lineups was 7.7 screens of scrolling for two leagues, a lineup row ran to eight lines and a league card to 2,603px, with the recommended list and the side-by-side comparison showing the same nine players twice. Now: a row carries the slot, the name, the kickoff and the rank, and its supporting lines (the game context, the why line, the rank note) appear only where a decision is being made, a change or a close call (`rowDepth`); the role flag rides the sub line as "▲ snaps"; the why line is a comparison rather than a sentence. Everything that explains the calls (close calls, disagreements, tilts, the wire) folds behind one line, and so does "Yours vs recommended"; a hurt starter stays in the open, being something to act on. **Exposure, Byes and Schedule strength became one Planning screen** with chips, taking the Players menu from five entries to three; their addresses still work and open Planning on that view. One fold style instead of two. The home page gained the six features it had grown since it was written. Result: 4.9 screens, a card of 1,391px, five lines a row |
| `v1.77.1` | Match Up data says when this week has no sheet. The screen opens on the newest week uploaded, so with only week 2 loaded in week 3 it would look current; a line now says the week is missing and that the calls fall back to the season's points allowed by position. No stale sheet ever feeds a call: `muWeek` is keyed strictly by the week being shown |
| `v1.77.0` | Three more, and one that turned out to be done already. **The disagreement notes are graded** like the close calls: `recordCalls` keeps each one before kickoff (keyed `n|...`), `SCC.gradeCalls` counts them in their own bucket, out of the close-call score, and the Results card shows "times the projections disagreed and your rankings were right" for the week and the season. That is how the three-point bar gets judged instead of assumed. **What your rankings would have started, beside what did** (`scoreCompare`, `scoreLeague` now returns `byRankLine`): spot by spot with the points each scored, the differing rows marked, and the gap at the foot, only once the week is over. **A pre-kickoff sweep** on Today (`SCC.lineupSweep`, pure and tested): one line saying how many lineups are set and which aren't, worst first, so a Sunday morning doesn't mean reading twelve cards. The fourth, re-reading players who changed team, is already what every refresh does (`finish` in sleeper.js re-reads each rostered player and saves the corrected team), so nothing was needed |
| `v1.76.0` | Injury tags on the live tick (the owner asked how often Titan checks). They were only re-read on a full refresh, so sitting on Lineups on a Sunday the screen ticked every minute but could miss Questionable turning into Out for half an hour. Now every fifth tick on a game day re-reads the players who already carry a tag (`SCC.taggedIds`, `SCC.applyInjuries`, both pure and tested; `API.refreshInjuries` does the fetching), updates `inj` and `outish` across every league that holds him, clears a tag that has gone, and the tick's own `analyze()` and repaint follow. A handful of calls rather than one per rostered player; a tag appearing from nothing still waits for the full refresh or the server's alert |
| `v1.75.0` | Three fixes to the matchup tilt, from a call the owner caught: in League 10 it started Zay Flowers (his rankings' WR59, rank 130, Questionable with a hamstring) over Dontayvion Wicks (WR46, rank 102, healthy), because the two share tier 8 and Sleeper projected Flowers six points higher. **A tier is no longer close on its own** (`closeByRank`): the same tier counts only when the two are also within `CLOSE` (12) ranks, since a 345-row file's tier holds dozens and the order inside it is still the ranker's opinion. **A hurt player is never tilted in over a healthy starter**, because the projection is written as though he plays. **And the tilt has to be about the matchup**: `analyzeLeague` takes `opts.rankOf` and flips only when the bench player's matchup rank is the softer one by `tiltFromRank`, so an identical matchup plus a projection gap changes nothing, which is the rankings' business. A flip records `muIn`/`muOut`, and the note on the card names both matchups. The case now shows as a "projections disagree" note instead, which also names the status. The app and the extension (v0.7.0) both pass `rankOf`; without it there are no flips at all |
| `v1.74.0` | Four the owner asked for. **Streamers this week** (Waivers, `streamCard`, `SCC.streamPicks` pure and tested): in every league that starts a kicker or a defense, the free agents at that position in the order this week's game suggests rather than their season rank, a kicker by his own team's expected points, a defense by how few the offense it faces is expected to score, both moved by the matchup rank, each with the bid, the starter he'd replace and a plain reason. **Why each recommended starter has his spot** (`whyStart`, `SCC.nextBest`): the best player on the bench who could have taken it and how far back he is ("a tier clear of X (WR47)"), or that there is no other fit, so a call can be checked instead of trusted. **A role on the move** (`roleFlag`): a lineup row says when a player's snap share is rising or falling against the weeks before, the same usage the waiver lists read; Lineups now loads it too. **The extension is renamed** to Titan Fantasy Assistant (v0.6.0): a product name carrying another service's mark implies an endorsement, which the Web Store treats as misleading, so "Sleeper" stays only where it states what the extension works with, and the listing says it is not affiliated. The site needed no change: every footer already carries that disclaimer |
